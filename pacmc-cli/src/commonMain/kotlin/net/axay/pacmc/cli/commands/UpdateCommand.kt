...
class UpdateCommand : CliktCommand(
    ...
    override fun run() = launchJob {
        ...
        updateResult.updateVersions.forEach {
            terminal.println(TextColors.brightGreen("↑") + " update " + it.files.primaryName())
        }
        updateResult.updateDependencyVersions.forEach {
            terminal.println("${TextColors.brightGreen("↑")} update ${it.files.primaryName()} ${TextColors.brightCyan("(dependency)")}")
        }
        updateResult.addedDependencyVersions.forEach {
            terminal.println("${TextColors.brightGreen("+")} add ${it.files.primaryName()} ${TextColors.brightCyan("(dependency)")}")
        }
